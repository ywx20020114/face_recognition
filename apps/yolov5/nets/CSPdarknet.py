...
from apps.yolov5.utils.utils import compose
...
#   单次卷积DarknetConv2D
#   如果步长为2则自己设定padding方式。 正则化方式改为l2
...
# CSP结构中的残差网络分为两个分支，
# 主干分支做一次1x1卷积和一次3x3卷积，残差边部分不做任何处理，相当于直接将主干的输入和输出结合。
...
def resblock_body(x, num_filters, num_blocks, expansion=0.5, shortcut=True, last=False, weight_decay=5e-4, name=""):
    ...
    return C3(x, num_filters, num_blocks, shortcut=shortcut, expansion=expansion, weight_decay=weight_decay,
...
def darknet_body(x, base_channels, base_depth, weight_decay=5e-4):
    ...
    x = resblock_body(x, base_channels * 16, base_depth, shortcut=False, last=True, weight_decay=weight_decay,
                      name='backbone.dark5')
    feat3 = x
